      ...
      try {
        const parseGeminiResponse = (responseText: string): AnalysisData => {
          ...
          const extractUrl = (text: string): string | undefined => {
            ...
            const match = text.match(/\[([^\]]+)\]\(([^)]+)\)/); // Matches [text](url)
            if (match) {
                return match[2]; // Return the URL part
            }
            // Fallback for direct URLs or links without markdown format
            const urlRegex = /(https?:\/\/[^\s]+)/;
            const directMatch = text.match(urlRegex);
            if (directMatch) {
                return directMatch[0];
            }
            return undefined; // No URL found
          };
          ...
          for (const line of lines) {
            ...
            if (trimmedLine.startsWith('1. **Seasonal Color Type:**')) {
              data.seasonType = trimmedLine.replace('1. **Seasonal Color Type:**', '').trim();
              currentSection = null;
            } else if (trimmedLine.startsWith('2. **Color Extraction:**')) {
              currentSection = 'colorExtraction';
            } else if (trimmedLine.startsWith('3. **9-Color Seasonal Palette:**')) {
              currentSection = 'colorPalette';
            } else if (trimmedLine.startsWith('4. **Jewelry Tone:**')) {
                const parts = trimmedLine.replace('4. **Jewelry Tone:**', '').trim().split(',').map(s => s.trim());
                if (parts.length >= 2) {
                    data.jewelryTone = { name: parts[0], hex: parts[1] };
                }
                currentSection = null;
            } else if (trimmedLine.startsWith('5. **2 Flattering Hair Colors:**')) {
                currentSection = 'hairColors';
            } else if (trimmedLine.startsWith('6. **Makeup Suggestions:**')) {
                currentSection = 'makeupSuggestions';
            } else if (trimmedLine.startsWith('- **Foundations:**')) {
                currentSection = 'foundations';
            } else if (trimmedLine.startsWith('- **Korean Cushion:**')) {
                currentSection = 'cushion';
            } else if (trimmedLine.startsWith('- **Lipsticks:**')) {
                currentSection = 'lipsticks';
            } else if (trimmedLine.startsWith('- **Blushes:**')) {
                currentSection = 'blushes';
            } else if (trimmedLine.startsWith('- **Eyeshadow Palettes:**')) {
                currentSection = 'eyeshadows';
            } else if (trimmedLine.startsWith('7. **2 Similar Celebrities:**')) {
                currentSection = 'celebrities';
            } else if (trimmedLine.startsWith('8. **Image Prompt:**')) { // Gemini 1.5 may also provide image prompt
                data.imagePrompt = trimmedLine.replace('8. **Image Prompt:**', '').trim();
                currentSection = null;
            }
            // Skip header lines like "Label,HEX" or "Name,HEX" or "Brand,Product,Shade,HEX,URL"
            ...
            else if (currentSection === 'colorExtraction') {
                const [label, hex] = trimmedLine.split(',').map(s => s.trim());
                if (label && hex) data.colorExtraction.push({ label, hex });
            } else if (currentSection === 'colorPalette') {
                const [name, hex] = trimmedLine.split(',').map(s => s.trim());
                if (name && hex) data.colorPalette.push({ name, hex });
            } else if (currentSection === 'hairColors') {
                const [name, hex] = trimmedLine.split(',').map(s => s.trim());
                if (name && hex) data.hairColors.push({ name, hex });
            } else if (currentSection === 'foundations') {
                const parts = trimmedLine.replace(/^- /, '').split(',').map(s => s.trim());
                const url = extractUrl(trimmedLine);
                if (parts.length >= 4) {
                    data.makeup.foundations.push({ brand: parts[0], product: parts[1], shade: parts[2], hex: parts[3], url });
                }
            } else if (currentSection === 'cushion') {
                const parts = trimmedLine.replace(/^- /, '').split(',').map(s => s.trim());
                const url = extractUrl(trimmedLine);
                if (parts.length >= 3) {
                    data.makeup.cushion = { brand: parts[0], product: parts[1], shade: parts[2], hex: parts[3] || undefined, url };
                }
            } else if (currentSection === 'lipsticks') {
                const parts = trimmedLine.replace(/^- /, '').split(',').map(s => s.trim());
                const url = extractUrl(trimmedLine);
                if (parts.length >= 3) {
                    data.makeup.lipsticks.push({ brand: parts[0], product: parts[1], shade: parts[2], hex: parts[3] || undefined, url });
                }
            } else if (currentSection === 'blushes') {
                const parts = trimmedLine.replace(/^- /, '').split(',').map(s => s.trim());
                const url = extractUrl(trimmedLine);
                if (parts.length >= 3) {
                    data.makeup.blushes.push({ brand: parts[0], product: parts[1], shade: parts[2], hex: parts[3] || undefined, url });
                }
            } else if (currentSection === 'eyeshadows') {
                const parts = trimmedLine.replace(/^- /, '').split(',').map(s => s.trim());
                const url = extractUrl(trimmedLine);
                // Eyeshadows sometimes don't have hex, so adjust parsing
                if (parts.length >= 2) {
                    data.makeup.eyeshadows.push({ brand: parts[0], product: parts[1], url: url });
                }
            } else if (currentSection === 'celebrities') {
                // Celebrities are typically just names, sometimes with a leading hyphen
                const name = trimmedLine.replace(/^- /, '').trim();
                if (name) data.celebrities.push(name);
            }
          }
          return data;
        };
        ...
        <div className="report-pdf" ref={reportRef}>
          ...
          <div className="p-6 text-[#3c3334] text-sm font-medium bg-[#FCF2DF] h-[calc(100vh-202px)] overflow-y-auto">
            ...
            <div className="mt-[50px] mb-8 text-center">
              <p className={styles.reportTitle}>
                STYLE: <span className="capitalize">{styleOption}</span>
              </p>
